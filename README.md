...
| `AGENTS.md` | The build posture: CPU budget, Swift 6 concurrency, gated Metal, premium aesthetic, proof-not-vibes, iOS 26/AI non-negotiables, and **§10 the fixture-prove rules for camera/scan/sensor features** (drive the inner loop off a fixture, not the human). `vibe` auto-loads it at a project root. | validated (loads, layers on vibe's prompt) |
...
| `patterns/FIXTURE-CAPTURE.md` | The debug capture seam — a `CaptureSource` protocol + `FixtureCaptureSource` so a camera/scan feature's inner loop runs against a bundled fixture (no camera, no human). Includes the honest fakeable / replayable / not-fakeable table (flat RGB vs recorded RGBD vs live ARKit/TrueDepth) and an XCTest harness. | APIs verified vs `apple-docs` |
| `patterns/VISUAL-VERIFY.md` | The **visual** analogue: how the agent SEES rendered SwiftUI without a human eyeballing it. The render→capture→**read-the-PNG-back-with-the-vision-tool**→golden-diff loop, the pass/fail checklist (no-emoji, token style, spacing), the `ImageRenderer` vs running-app-screenshot table, and honest limits. Wired into `AGENTS.md §11`. | APIs verified vs `apple-docs` / SDK |
| `templates/VisualSnapshot.swift` | `ImageRenderer`-based snapshotter: render a view to a deterministic PNG (`proposedSize`/`scale` pinned) + `GoldenDiff` AA-tolerant pixel compare against a committed reference. | **type-checks clean** vs the iOS 27 SDK |
| `templates/SnapshotHarness.swift` | An XCTest that hosts each component, writes `<name>.png`, attaches it to the `.xcresult`, prints the path for the agent to open, and asserts the golden diff. The agent's visual inner loop. | machinery **type-checks clean** vs the iOS 27 SDK (only the `XCTAssert` C-macro is unexpandable by standalone `swiftc`; fine under `xcodebuild test`) |
